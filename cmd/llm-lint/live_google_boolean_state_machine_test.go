...
// R-4JXW-H40T
func TestRunLiveGoogleReportsBooleanStateMachine(t *testing.T) {
	if os.Getenv("GEMINI_API_KEY") == "" {
		t.Skip("GEMINI_API_KEY is not set")
	}

	cwd := filepath.Join("..", "..", "testdata", "live-boolean-state-machine")
	...
	for _, want := range []string{"ticket_state.go:", "(boolean-state-machine)"} {
